...
- Would a value object make meaning, constraints, units, comparison, or behavior explicit? Introduce one within the touched design when it owns a real invariant or behavior, makes units explicit, or prevents repeated misuse, and when that benefit exceeds its integration cost.
...
- Is each failure represented so callers can handle it correctly, with business rejections distinguishable from defects and infrastructure failures while preserving established ecosystem and public error contracts?
